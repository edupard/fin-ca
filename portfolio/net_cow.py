...
class NetCow(object):
    def __init__(self):
        self.variances = variances= tf.placeholder(tf.float32, [None], name='variances')
        ...
        for num_units in get_config().FC_LAYERS_SIZE:
            ...
            with tf.name_scope(scope_name):
                h1 = tf.contrib.layers.fully_connected(x, num_units, activation_fn=None,
                                                                 scope='dense_%d' % idx)
                if get_config().BATCH_NORM:
                    ...
                    h2 = h1
                x = tf.nn.tanh(h2, 'tanh_%d' % idx)
            idx += 1

        # final fc layer
        scope_name = 'fc_layer_%d' % idx
        with tf.name_scope(scope_name):
            x = tf.contrib.layers.fully_connected(x, 1, activation_fn=None, scope='dense_%d' % idx)
            x = tf.reshape(x,[-1])

        with tf.name_scope('loss'):
            self.returns = x
            diff = self.returns - labels
            self.cost = tf.reduce_mean(tf.square(diff) / variances)
    ...
    def fit(self, x, labels, variances):
        self.sess.run((self.optimizer), feed_dict={self.x: x, self.labels: labels, self.variances: variances, self.phase: True})

    def eval(self, x, labels, variances):
        cost, returns = self.sess.run(
            (self.cost, self.returns),
            feed_dict={self.x: x, self.labels: labels, self.variances: variances, self.phase: False})
        return cost, returns
