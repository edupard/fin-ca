...
class NetApple(object):
    def __init__(self):
        ...
        scope_name = 'fc_layer_%d' % idx
        with tf.name_scope(scope_name):
            h1 = tf.contrib.layers.fully_connected(x, 2, activation_fn=None, scope='dense_%d' % idx)
            if get_config().BATCH_NORM:
                h2 = tf.contrib.layers.batch_norm(h1,
                                                  center=True, scale=True,
                                                  is_training=phase,
        ...
        with tf.name_scope('loss'):
            ...
            buy_feature = tf.gather(x, [0], name='buy_feature')
            sell_feature = tf.gather(x, [1], name='sell_feature')

            buy_prob = tf.nn.softmax(buy_feature, name='buy_prob')
            sell_prob = tf.nn.softmax(sell_feature, name='sell_prob')
            self.weights = weights = tf.reshape((buy_prob - sell_prob) / 2, [-1])
